(function () {
  ...
  var STYLE_ID = "autosure-native-capsule-style";
  var MENU_ID = ROOT_ID + "-menu";
  var BTN_ID = ROOT_ID + "-trigger";
  var LEGACY_OVERLAY_SELECTORS = ["#autosure-capsule-root", "#autosure-probe-fixed-18789"];
  var DEBUG = window.localStorage && window.localStorage.getItem("autosure.native.debug") === "1";
  var ENABLE_OBSERVER = window.localStorage && window.localStorage.getItem("autosure.native.enableObserver") === "1";
  var ENABLE_AUTO_STATUS = window.localStorage && window.localStorage.getItem("autosure.native.enableAutoStatus") === "1";
  var DISABLE_SUPPRESS = window.localStorage && window.localStorage.getItem("autosure.native.disableSuppress") === "1";
  ...
  var visibleInjectConsumerId = "autosure-native-capsule";
  ...
  function debugLog() {
    if (!DEBUG) return;
    try {
      console.log.apply(console, ["[autosure-native]"].concat(Array.prototype.slice.call(arguments)));
    } catch (e) {}
  }
  ...
  function generateRunId() {
    ...
    return "autosure-" + Date.now() + "-" + Math.random().toString(16).slice(2, 10);
  }

  function reportSendFailure(error) {
    var message = error && error.message ? error.message : String(error || "unknown error");
    debugLog("send failure", message);
    updateStatus("error", "发送失败");
  }

  function sendCommandViaHost(cmd) {
    var app = getOpenClawApp();
    if (!app) return false;

    if (typeof app.handleSendChat === "function") {
      try {
        var result = app.handleSendChat(cmd);
        if (result && typeof result.then === "function") {
          result.catch(reportSendFailure);
        }
        debugLog("sendCommand:host.handleSendChat", cmd, app.sessionKey || "");
        return true;
      } catch (e) {
        debugLog("sendCommand:host.handleSendChat failed", e && e.message ? e.message : String(e));
      }
    }

    if (app.client && typeof app.client.request === "function" && app.connected && app.sessionKey) {
      try {
        var req = app.client.request("chat.send", {
          sessionKey: app.sessionKey,
          message: cmd,
          deliver: false,
          idempotencyKey: generateRunId(),
        });
        if (req && typeof req.then === "function") {
          req.catch(reportSendFailure);
        }
        debugLog("sendCommand:host.client.request", cmd, app.sessionKey);
        return true;
      } catch (e) {
        debugLog("sendCommand:host.client.request failed", e && e.message ? e.message : String(e));
      }
    }
  ...
  function sendCommandViaComposer(cmd) {
    var ta = findTextarea();
    ...
    btn.click();
    debugLog("sendCommand:composer", cmd);
    return true;
  }

  function sendCommand(cmd) {
    if (sendCommandViaHost(cmd)) return true;
    if (sendCommandViaComposer(cmd)) return true;
  ...
  function getActiveSessionKey() {
    var app = getOpenClawApp();
    if (app && typeof app.sessionKey === "string" && app.sessionKey) return app.sessionKey;
    try {
  ...
  function sendVisiblePhrase(phrase) {
    ...
    if (sendCommandViaComposer(text)) {
      return Promise.resolve({ ok: true, via: "composer" });
    }

    if (app && typeof app.handleSendChat === "function") {
      try {
        var result = app.handleSendChat(text);
        return Promise.resolve(result).then(function () {
          return { ok: true, via: "handleSendChat" };
        });
      } catch (e) {
        debugLog("sendVisiblePhrase:handleSendChat failed", e && e.message ? e.message : String(e));
      }
    }
  ...
  function injectStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      "#autosure-capsule-root,#autosure-probe-fixed-18789{display:none !important;pointer-events:none !important;opacity:0 !important;}",
  ...
  function suppressLegacyOverlays() {
    if (DISABLE_SUPPRESS) return;
    LEGACY_OVERLAY_SELECTORS.forEach(function (selector) {
      document.querySelectorAll(selector).forEach(function (el) {
        el.setAttribute("data-autosure-suppressed", "native-fallback");
